...
const App = (() => {
  ...
  function submitActividad(e) {
    ...
    if (id) { DB.updateActivity(id, data); showToast('Actividad actualizada', 'success'); }
    else    { DB.addActivity(data);        showToast('Actividad creada',       'success'); }
    closeModal();
    UI.renderActividades();
    UI.renderDashboard();
  }

  function toggleCompletado(id, checked) {
    DB.updateActivity(id, { estado: checked ? 'completado' : 'pendiente' });
    UI.renderActividades();
    UI.renderDashboard();
  }
  ...
  function submitContacto(e) {
    ...
    if (id) { DB.updateContact(id, data); showToast('Contacto actualizado', 'success'); }
    else    { DB.addContact(data);        showToast('Contacto creado',       'success'); }
    closeModal();
    UI.renderContactos();
  }
